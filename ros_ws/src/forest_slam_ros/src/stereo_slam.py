...
def stereo_slam():
    ...
    bag = rosbag.Bag(os.path.join(os.path.dirname(__file__), '../../Datasets/BotanicGarden/1018_00_img10hz600p.bag'))
    ...
    # # Initialise ORB detector and brute force matcher
    # orb = cv2.ORB_create()
    ...
    for index, (topic, msg, t) in enumerate(bag.read_messages(topics=['/dalsa_rgb/left/image_raw', '/dalsa_rgb/right/image_raw'])):
        ...
        if topic == '/dalsa_rgb/left/image_raw':
            ...
            cur_img_left_blurred = apply_random_motion_blur(cur_img_left, blur_percentage=0, kernel_size=20, angle=0)
        ...
        # get right img and perform stereo SLAM
            ...
            # apply random motion blur
            cur_img_right_blurred = apply_random_motion_blur(cur_img_right, blur_percentage=0, kernel_size=20, angle=0)
            ...
            if index % frame_interval == 0:
                if prev_img_left is not None and prev_img_right is not None:
                    ...
                    # uncomment the following for ORB feature extraction and matching
                    ...
                    # uncomment the following to visualise the feature matches
                    ...
                    if len(points3D) >= 6:
                        ...
                        transformed_points = np.matmul(cumulative_est_tf_mat, homogeneous_points3D.T)
    ...
    # uncomment the following to save the data to the results folder
